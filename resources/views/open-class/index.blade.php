...
<main class="bg-naomi-bg min-h-screen">
    ...
    <section class="py-14 lg:py-20 relative overflow-hidden">
        ...
        <div class="max-w-7xl mx-auto px-6 text-center relative z-10">
            <span class="inline-block px-5 py-2 bg-naomi-white border border-primary/20 text-primary rounded-full text-[10px] font-black uppercase tracking-[0.3em] mb-5 shadow-sm">
                Join the Movement
            </span>
            <h1 class="serif-title text-5xl md:text-7xl font-black text-charcoal mb-4 tracking-tight">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            ...
            <div class="group bg-naomi-white rounded-[2.5rem] overflow-hidden shadow-sm hover:shadow-2xl transition-all duration-500 border border-charcoal/5 flex flex-col">
                ...
                <div class="p-7 flex flex-col flex-grow">
                    <div class="flex items-center gap-2 text-[11px] font-black text-primary mb-1.5 uppercase tracking-[0.1em]">
                        <span class="material-symbols-outlined text-sm">schedule</span>
                        {{ $class->day_of_week }} • {{ substr($class->time_start, 0, 5) }} WIB
                    </div>
                    <div class="text-[10px] text-slate-500 mb-1.5 uppercase tracking-[0.2em] font-black">
                        {{ $class->class_date ? $class->class_date->format('d M Y') : 'Tanggal belum ditetapkan' }}
                    </div>

                    <h3 class="serif-title text-2xl font-bold text-charcoal mb-1.5 group-hover:text-primary transition-colors tracking-tight">
                        {{ $class->title }}
                    </h3>

                    <div class="flex items-center gap-2 text-[10px] font-bold text-charcoal/40 mb-3">
                        <span class="material-symbols-outlined text-xs">person</span>
                        <span>Mentor: {{ $class->mentor ? $class->mentor->name : $class->instructor_name }}</span>
                    </div>

                    <p class="text-charcoal/60 text-sm font-light leading-relaxed mb-4 flex-grow">
                        {{ $class->description }}
                    </p>

                    <div class="flex items-center justify-between gap-4 mb-4">
                        <div>
                            <p class="text-[10px] uppercase tracking-[0.2em] text-slate-500 mb-0.5">Early Bird</p>
                            <p class="text-sm text-slate-600">{{ $class->early_bird_price ? 'Rp' . number_format($class->early_bird_price, 0, ',', '.') : '-' }}</p>
                        </div>
                        <div class="text-right">
                            <p class="text-[10px] uppercase tracking-[0.2em] text-slate-500 mb-0.5">D-Day</p>
                    ...
                    <div class="pt-3 border-t border-charcoal/5">
                        <a href="{{ route('open-class.show', $class->slug) }}"
                           class="btn-main w-full py-3 rounded-2xl text-xs font-black uppercase tracking-[0.2em] text-center block shadow-lg">
                            Lihat Detail & Daftar
                        </a>
                    </div>
                </div>
