...
class LaporanController extends Controller
{
    protected function buildReportData(Request $request): array
    {
        ...
        for ($i = 5; $i >= 0; $i--) {
            ...
            $chartIncome[] = Payment::where('status', 'verified')
                ...
                ->whereYear('paid_at', $y)
                ->sum('amount');
            $chartExpense[] = OperationalCost::where('period_month', $m)
                ->where('period_year', $y)
                ->sum('amount');
        }
    ...
    public function exportPdf(Request $request)
    {
        ...
        $pdf = \Barryvdh\DomPDF\Facade\Pdf::loadView('pdf.laporan', $data)->setPaper('a4', 'portrait');

        $filename = 'laporan-' . date('F', mktime(0, 0, 0, $data['month'], 1)) . '-' . $data['year'] . '.pdf';
